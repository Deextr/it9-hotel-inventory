        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('inventory.purchase_orders.store') }}" method="POST" x-data="purchaseOrderForm()">
                        @csrf
                                ...
                                <template x-for="(item, index) in items" :key="index">
                                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 pb-4 border-b border-gray-200">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" :name="'items['+index+'][quantity]'" x-model="item.quantity" min="1" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required @input="calculateSubtotal(index)">
                                        </div>
                                        <div>
                                            <label :for="'items['+index+'][unit_price]'" class="block text-sm font-medium text-gray-700">Unit Price</label>
                                            <input type="number" :name="'items['+index+'][unit_price]'" x-model="item.unit_price" min="0" step="0.01" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required @input="calculateSubtotal(index)">
                                        </div>
